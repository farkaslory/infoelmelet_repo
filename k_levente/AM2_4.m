...
% AM2_4 Szimulációs mag az LDPC kódoláshoz (Párhuzamosított változat)
% Bemenetek:
...
for snr_idx = 1:length(snrRange)
    ...
    max_err_pos = zeros(1, maxFrames); % Ideiglenes változó a parfor miatti place számításhoz
    
    parfor k_idx = 1:maxFrames
        % Változók lokális inicializálása a worker szálakhoz
        data = cell(1, K);
        codedData = cell(1, K);
        cD1 = cell(1, K);
    ...
    % A place vektor frissítése a parfor cikluson kívül
    if calcPlace
