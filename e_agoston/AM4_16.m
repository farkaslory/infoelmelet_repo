...
if nargin < 4 || isempty(snr_range),  snr_range  = 1:0.5:5;  end
if nargin < 3 || isempty(K),         K         = 20;       end
if nargin < 2 || isempty(numTrials), numTrials = 1000;     end
...
%% --- Elofoglalas --------------------------------------------------------
LdpcErr_2 = zeros(1, numel(snr_range));
if calcPlace
    place = zeros(1, K);
else
    place = [];
...
for snr_idx = 1:numel(snr_range)
    ...
    error_count = zeros(1, numTrials);

    for k_idx = 1:numTrials
    ...
    LdpcErr_2(snr_idx) = mean(error_count) / n;
end
